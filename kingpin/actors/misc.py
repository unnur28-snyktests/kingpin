...
class Macro(group.Macro):

    """Place holder for misc.Macro actor to transition to group.Macro"""

    @gen.coroutine
    def _execute(self):

        self.log.warning(
            'WARNING! `misc.Macro` has been moved to `group.Macro` '
            ' and will not be supported in the future.')

        yield super(Macro, self)._execute()
